...
```python
from src.core.workflows.stage_tracking import make_tracker, track_target
...
### Bacteria — Run-and-Tumble (ch552=6/10)
- Very small at low mag (1-3 px at 10x); use 20x+ for tracking
- Use GFP/fluorescence, NOT BF (BF creates ghost detections from phase halos)
- MDA timelapse: 40+ frames, 0.3s interval
- **Tracking**: `build_tracks(positions, max_dist=20, min_length=5, max_gap=1)`
  - Gap filling (`max_gap=1`) prevents fragmentation from detection dropouts
  - **Sanity-check**: `validate_tracks(tracks, n_visible=15)` from `src.core.analysis.run_tumble`
    - Warns if tracks > 3× visible count (fragmentation detected)
    - Without gap filling: ch552 got 166 tracks for 60 bacteria (2.8x fragmentation)
- **Run-tumble classification**: `analyze_run_tumble()` from `src.core.analysis.run_tumble`
  - **Angle threshold ≥90°** (default now 90°): real tumbles average ~68° reorientation
  - 45° was too sensitive — centroid noise (±1px on ~6.6px moves = ~17° apparent angle)
  - **min_tumble_frames=2**: require ≥2 consecutive tumble-like frames (suppresses noise)
  - E. coli: mean run ~1s, mean tumble ~0.1s → ~91% time running
- **Report run speed specifically**: `mean_run_speed` (not `mean_all_speed` which includes tumbles)
...
- `src/analysis/run_tumble.py`: build_tracks, analyze_run_tumble, aggregate_run_tumble
