...
## Analysis Approaches
- **Wave analysis**: Use `src.core.analysis.temporal` (FFT, autocorrelation) on membrane channel
...
- **Streaming detection**: Track cell displacement vectors over time
  - Use `src.core.analysis.tracking.match_centroids()` for frame-to-frame matching
...
- **Bleach inflates σ**: cAMP-reporter channels bleach over a long burst; raw `stack.std(axis=0)` and `σ × mean` then rank bleach-decay pixels above real wave-source candidates. For aggregation-centre / pacemaker localisation prefer `src.core.utils.firing_energy.firing_energy(stack, diff_threshold=10)` (positive-rise sum) or `detrended_sigma(stack)` (per-pixel σ after subtracting linear time trend) — both are bleach-immune. ch651 r3 cardio lesson; same trap applies any time the channel has a global decay envelope.
...
## ch550 Lessons — cAMP Wave Velocity (8/10)

### Best approach: BF timelapse + FFT frequency/phase analysis
1. **Identify pacemakers** from membrane channel (bright cAMP reporter spots, threshold >80-100)
2. **BF timelapse** (60 frames, 1s intervals) — BF shows wave rings as optical density changes
3. **`frequency_map()`** from `optical_mapping.py` → dominant frequency = pulse period
   - Use `min_freq=0.05, max_freq=0.3` to avoid trend capture
   - Verify with ring traces at r=50-70 from each pacemaker
4. **`phase_map()` + `conduction_velocity()`** → wave speed = omega × (1/|grad_phase|)
   - Use `block_size=16` for finer spatial resolution
   - wave_speed = 2π × freq × cv_median
5. **Ring trace FFT** for per-pacemaker frequency verification

### Key findings
- Membrane channel: shows pacemaker CENTERS as steady bright spots, NOT the propagating waves.
  The initial large blob is a transient pulse, not steady-state oscillation.
- BF channel: shows wave rings as optical density change (cells shape change on cAMP arrival).
  Use BF for wave dynamics, membrane for pacemaker identification.
- Wave speed ~30 px/s, period ~10s is typical for this simulation
- `frequency_map()` mode can be unreliable if most blocks are empty (cells sparse).
  Filter by power and use ring-trace FFT as verification.
- Cross-correlation between radial rings has sub-frame resolution if wave is fast (~30 px/s).
  Phase-based analysis is more robust than cross-correlation for fast waves.
