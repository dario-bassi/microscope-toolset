...
**Assumes:** FUCCI-expressing mammalian cells, 2-channel fluorescence (nucleus + geminin), 10x/20x for population counts. Numeric per-phase intensities below are sim-specific — the relative ordering transfers but the values will not. On a real prep, re-measure them on known populations and set your own thresholds.
...
| Phase | nucleus-channel (sim) | geminin-channel (sim) | Detection |
|-------|----------------|-----------------|-----------|
...
```python
from src.core.analysis.cell_cycle import detect_fucci_division
...
```python
from src.core.analysis.cell_cycle import detect_fucci_g2m
...
## Multi-Well Population FUCCI Survey (ch547, 7/10)

### Different Challenge Type
When FUCCI has nucleus-channel (Cdt1) + membrane-channel (Geminin in membrane),
classification is **population-level** not single-cell tracking.

### Cell Segmentation from Membrane Boundaries
```python
# Membrane-channel shows Voronoi-like cell boundaries
mem_smooth = gaussian_filter(mem_img.astype(float), sigma=1)
boundary_thresh = np.percentile(mem_smooth[mem_smooth > 3], 80)
boundary_mask = mem_smooth > boundary_thresh
cell_regions = ~boundary_mask  # cells are inside boundaries
cell_regions = binary_opening(cell_regions, iterations=1)
labeled, n = label(cell_regions)
```

### FUCCI Phase Classification (multi-well)

**WRONG approach (ch547)**: Global percentile thresholds (p25, p75, median)
- Gives biased MI values, noisy between wells

**BETTER approach**: Classify M-phase first with extreme thresholds
```python
# M-phase = very dim nucleus AND very bright membrane
# Use bottom 10% nuc + top 10% mem
m_phase = (nuc_val < np.percentile(all_nuc, 10)) and \
          (mem_val > np.percentile(all_mem, 90))

# OR ratio-based: nuc/mem ratio < 0.1
ratio = nuc_val / (mem_val + 1e-6)
m_phase = ratio < 0.1

# Then classify remainder
g1 = nuc_val > np.median(all_nuc)  # G1 = bright nucleus
g2 = mem_val > np.median(all_mem)  # G2 = bright membrane, but not extreme
s_phase = not m_phase and not g1 and not g2  # S = moderate both
```

### 40x Verification Step
- **CRITICAL**: Use `set_objective_verified(core, 40)` from `src.core.hardware.core`
  - Auto-verifies pixel size changed to 0.25 µm/px after switch
  - Raises RuntimeError if verification fails within 2 seconds
  - At 40x, FOV = 128x128 world pixels (only 1-2 cells visible)
  - If you still see 10+ cells, the objective didn't switch (ch547: cost 2 pts)

### Key Lessons
- Mitotic index per well: `MI = n_M_phase / n_total`
- For multi-well comparison, use consistent thresholds across wells
- Use `pairwise_comparisons()` from statistics module for rigorous comparison
