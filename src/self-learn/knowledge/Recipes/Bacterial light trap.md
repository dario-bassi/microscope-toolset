...
**Assumes:** SLM-equipped scope, motile photosensitive bacteria (E. coli expressing photoreceptor or similar), a target region defined in the challenge/experiment brief. Speed factor 0.15 and "~3× enrichment after 40 steps" are simulator-calibrated numbers; a real bacterial trap is tuned per strain/photoreceptor.
...
- Speed factor: 0.15 (85% slower inside SLM circle) *(sim-specific)*
- Result: bacteria accumulate passively → density inside > density outside
- Expected enrichment: ~3× after 40 steps at 10x *(sim-specific; real values depend on bacterial speed, photoreceptor, light intensity)*
...
```python
from src.recipes.bacteria_trap import detect_bacteria_gfp, compute_enrichment
...
```python
from src.core.hardware.core import make_slm_circle
...
### Correct method:
```python
from src.recipes.bacteria_trap import detect_bacteria_area_threshold, count_bacteria_in_circle
...
```python
from src.recipes.bacteria_trap import measure_bacteria_intensity
