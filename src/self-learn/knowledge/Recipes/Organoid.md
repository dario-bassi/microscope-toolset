...
from src.core.analysis.ring import measure_ring
...
### Z-Stack Morphometry Protocol (ch551=7/10)
1. **Coarse Z-scan** at 10x: BF, -20 to +140 µm, step 10 µm → find max cross-section area
2. **Fine Z-scan**: ±20 µm around coarse peak, step 2 µm → equatorial Z
3. **Switch to 20x** for precise measurements (organoid ~200µm fits in 256µm FOV)
4. **Membrane morphometry**: threshold E-cadherin > 5 → closing(disk=1) → fill_holes
   - Outer boundary: regionprops on filled mask (equiv_diameter)
   - Lumen: outer_filled & ~membrane_threshold → largest CC
   - Wall thickness: (outer_d - lumen_d) / 2
5. **Nuclei counting**: DAPI threshold > 12 → watershed (**min_distance=5-6**, not 9!)
   - min_distance=9 gave 48 vs GT 85 (44% undercount)
   - Nuclei in annular rings are closely packed (~6-8 px apart at 20x)
   - Filter to wall band only (outer_mask & ~lumen_mask)
6. **Bud counting**: use **membrane channel at 20x** (not BF at 10x)
   - E-cadherin highlights bud protrusions clearly
   - BF at 10x missed 1 of 2 buds
7. Report all in camera pixels at measurement objective
