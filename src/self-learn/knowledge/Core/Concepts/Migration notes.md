...
Old code may call:
```python
from src.core.workflows.mda import execute_mda
...
Old code may call:
```python
from src.core.hardware.core import run_mda
...
Old code may call:
```python
from src.core.workflows.bacteria_trap import run_bacteria_trap   # MODULE no longer exists
result = run_bacteria_trap(core, cx, cy, radius, ...)
```

**Replace with `run_bacteria_trap_mda()` from the new location:**
```python
from src.recipes.bacteria_trap import run_bacteria_trap_mda
result = run_bacteria_trap_mda(core, cx, cy, radius, ...)
```

The module moved from `src.core.workflows/` to `src.recipes/` during
the two-tier core/recipes split (post-restart sprint #4); the
function rename happened in the same pass.
