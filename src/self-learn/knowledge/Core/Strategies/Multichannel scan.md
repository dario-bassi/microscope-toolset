...
```python
from src.core.workflows.batch import multichannel_scan
...
- `src.core.workflows.batch.tile_and_analyze` — grid-based single-channel
  scan (computes positions from a `(rows, cols)` grid).
- `src.core.workflows.multi_scale.multi_position_events` — lower-level
  helper that just builds the event list (no acquisition, no analysis).
- `src.core.hardware.config.resolve_channel_group(core, group)` — used
  internally by multichannel_scan to default `group=None` to the runtime
  channel group.

## References

- Implementation: `src/core/workflows/batch.py::multichannel_scan`.
- First adopter: `scratch/solve_568.py` (hepatocyte steatosis,
  DAPI+bodipy+membrane at 10x).
