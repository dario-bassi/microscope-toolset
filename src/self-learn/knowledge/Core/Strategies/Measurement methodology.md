...
- Use `src.core.utils.fft_peak.find_fft_peak(image)` for single-orientation
...
- Use `src.core.utils.fluorophore_brightness.predict_brightness_ranking`
...
- Same primitive transfers to bleach-rate ranking (registry's
  ...
  REUSABLE per ch617 grader. Compose with `src.core.utils.spectral_leak`
...
- Use `measure_growth_rate_series(counts)` from `src.core.analysis.kinetics`
...
```python
from src.core.analysis.kinetics import measure_growth_rate_series
