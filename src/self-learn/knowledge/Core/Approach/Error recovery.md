...
## Principle: retry is cheap, wrong answers are expensive

A single suspicious detection is a signal to change something — refocus, switch channel, adjust exposure, try a different detection method — not to submit. Three attempts with different approaches cost seconds to minutes of acquisition; a wrong answer costs a grade and a post-mortem. Build closed-loop retry into every workflow: if detection returns 0 or a suspicious count, systematically vary the parameter most likely to be the cause and re-run.
