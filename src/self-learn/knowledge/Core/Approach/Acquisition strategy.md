...
well-exposed frame; they only consume photobleaching budget. ch602
(10/10) closed this archetype — see [[Core/Strategies/Gentle imaging]]
"Active Dose Budgeting" for the four-rule framework.
...
auto-tuner — `src.core.utils.wave_period.estimate_wave_period(core,
...
recipe `event_driven_modality_switch.modality_switch_pipeline`
...
utilisation isn't always achievable, but 50 % guards against an
unexpected stage-move or focus-recovery snap pushing into a hard
budget gate that triggers `BudgetExceededError`.
