...
- **Use `doubling_time()` from `src.analysis.confluency`** for automated fitting
- **Use `growth_curve()` from `src.analysis.confluency`** for logistic/exponential models
...
- **Temperature experiments**: use `core.setProperty("Temperature", "Label", "37")` for temp control
  ...
  - Use `fit_q10()` from `src.analysis.kinetics` for Q10 fitting
  - Use `growth_curve()` and `doubling_time()` from `src.analysis.confluency`
...
- Use `population_speeds()` from `src.analysis.tracking` for convenience
