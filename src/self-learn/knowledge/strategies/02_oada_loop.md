...
For SLM-based closed-loop control (using native `slm_image` on MDAEvent):

```python
from useq import MDAEvent, SLMImage
from src.workflows.oada import adaptive_generator

def slm_oada(core, target_pos, max_steps=30):
    """Closed-loop SLM via OADA — mask set per-event, not manually."""
    mask = create_slm_mask(target_pos)
    slm = SLMImage(data=mask.astype(np.uint8), device="SLM")
    base = [MDAEvent(channel={"config": "BF"}, slm_image=slm) for _ in range(max_steps)]

    def on_decide(image, event, state):
        current_pos = detect_object(image)
        distance = np.linalg.norm(np.array(current_pos) - np.array(target_pos))

        # DECIDE
        if distance < threshold:
            state.stop = True
        # For dynamic masks: inject updated event
        # new_mask = create_slm_mask(current_pos)
        # state.extra_events.append(MDAEvent(slm_image=SLMImage(data=new_mask, device="SLM")))
