...
def validate_cell_count(count, fov_area_um2, cell_type='generic',
                        magnification=None):
    ...
    if count == 0:
        ...
        warnings.append(f"Count {count} below expected minimum "
                       f"{expected_count_min} for {cell_type}")
    elif count > expected_count_max:
        warnings.append(f"Count {count} above expected maximum "
                       f"{expected_count_max} for {cell_type}")

    if magnification and magnification >= 40 and count > 200:
        warnings.append(f"High count ({count}) at {magnification}x — "
                       "verify not overcounting noise/texture")
...
def validate_size(areas_um2, cell_type='generic'):
    ...
    if len(areas) == 0:
        return {'valid': False, 'median_area': 0, 'expected_range': (0, 0),
                'fraction_outliers': 1.0, 'warnings': ['No objects to validate']}
    ...
    warnings = []
    if not valid:
        warnings.append(f"Median area {median_area:.1f} µm² outside expected "
                       f"range [{min_a}, {max_a}] for {cell_type}")
    if frac_outliers > 0.3:
        warnings.append(f"{frac_outliers:.0%} of objects are size outliers — "
                       "check threshold or segmentation")
...
def validate_speed(speeds, organism='generic', pixel_size=1.0, dt=1.0):
    ...
    if len(speeds) == 0:
        return {'valid': False, 'median_speed': 0,
                'expected_range': (0, 0), 'warnings': ['No speeds']}
    ...
    warnings = []
    if not valid:
        warnings.append(f"Median speed {median_speed:.1f} µm/s outside "
...
def validate_frequency(freq_hz, signal_type='generic'):
    ...
    warnings = []
    if not valid:
        warnings.append(f"Frequency {freq_hz:.3f} Hz outside expected "
                       f"[{min_f}, {max_f}] for {signal_type}")
...
def validate_concentration(count, volume_ul=None, dilution_factor=1.0):
    ...
    if volume_ul is None or volume_ul <= 0:
        return {'concentration': 0, 'valid': False,
                'warnings': ['No volume specified']}
    ...
    warnings = []
    if conc < 1e3:
        warnings.append(f"Very low concentration ({conc:.0f}/mL) — "
                       "check for under-counting")
    if conc > 1e8:
        warnings.append(f"Very high concentration ({conc:.0e}/mL) — "
...
def run_sanity_checks(measurements, fov_um2=None, pixel_size=None,
                      cell_type='generic'):
    ...
    if 'count' in measurements and fov_um2:
        result = validate_cell_count(
            measurements['count'], fov_um2, cell_type)
