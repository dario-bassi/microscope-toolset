...
def local_binary_pattern(image, radius=1, n_points=8):
    ...
    if h < 2 * margin + 1 or w < 2 * margin + 1:
        return {'histogram': np.array([1.0]),
                'n_patterns': 1, 'uniformity': 1.0}
    ...
    for code in unique_codes:
        ...
        transitions = sum(1 for a, b in zip(bits, bits[1:] + bits[0])
                          if a != b)
        if transitions <= 2:
            n_uniform += 1
...
def texture_map(image, window_size=15, feature='std'):
    ...
    if feature == 'std':
        ...
        raise ValueError(f"Unknown feature: {feature}. "
                         f"Use 'std', 'entropy', 'range', or 'contrast'.")
...
def entropy_map(image, window_size=9, n_bins=32):
    ...
    """
    return _local_entropy(np.asarray(image, dtype=np.float64),
                          window_size, n_bins)
    ...
    if isinstance(stain, str):
        if stain not in STAIN_MATRICES:
            raise ValueError(f"Unknown stain: {stain}. "
                             f"Available: {list(STAIN_MATRICES.keys())}")
