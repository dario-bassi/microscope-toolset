...
def measurement_cv(values):
    ...
    if n < 2:
        return {
            'cv': 0.0, 'cv_percent': 0.0,
            'mean': float(values[0]) if n == 1 else 0.0,
            'std': 0.0, 'n': n, 'quality': 'insufficient',
        }
...
def recount_variability(counts, method_names=None):
    ...
    methods = []
    for i, (name, count) in enumerate(zip(method_names, counts)):
...
def measurement_summary(values, label='measurement', target_cv=10.0):
    ...
    if n == 0:
        return {
            'label': label, 'n': 0, 'mean': 0, 'std': 0, 'se': 0,
            'cv_percent': 0, 'ci_95': (0, 0),
            'quality': 'no data', 'recommendation': 'Acquire measurements.',
        }
    ...
    if n < 3:
        ...
        rec = (f"CV={cv_result['cv_percent']:.1f}% exceeds target {target_cv}%. "
               f"Need ~{samp_result['n_needed']} samples or reduce measurement noise.")
    elif not samp_result['sufficient']:
        rec = (f"Precision {samp_result['current_precision']*100:.1f}% not yet at 5% target. "
