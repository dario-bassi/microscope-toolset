...
def radial_density_profile(image, center=None, max_radius=None,
                           n_bins=50, mask=None):
...
def steepest_gradient(radii, profile, direction='decreasing',
                      smooth_window=3):
...
def half_max_radius(radii, profile, baseline_fraction=0.5):
    ...
    if crossing_idx >= 0:
        ...
        f = (target - profile[crossing_idx]) / (
            profile[crossing_idx + 1] - profile[crossing_idx])
        interp_radius = radii[crossing_idx] + f * (
            radii[crossing_idx + 1] - radii[crossing_idx])
        return {
...
def wavefront_radius(stack, center=None, threshold=None,
                     n_bins=50, max_radius=None):
...
def annular_statistics(image, center=None, inner_radius=0,
                       outer_radius=None, n_rings=5, mask=None):
