...
def check_focus_quality(core, z_range=5.0, z_step=1.0):
    ...
    z_positions = np.arange(current_z - z_range / 2, current_z + z_range / 2 + z_step / 2, z_step)
    ...
    events = [MDAEvent(z_pos=float(z), index={"z": i}) for i, z in enumerate(z_positions)]
    ...
    improvement = (
        best_sharpness / current_sharpness if current_sharpness and current_sharpness > 0 else 1.0
    )
