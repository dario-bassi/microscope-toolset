...
def create_mcp_server(
    ...
    def pymmcore_api_database(
        ...
        try:
            ...
            if (
                isinstance(reformulated_question, dict)
                and reformulated_question.get("intent") == "error"
            ):
                result = {
                    "user_query": user_query,
            ...
            reformulated_query_str = (
                reformulated_question.get("reformulated_query", user_query)
                if isinstance(reformulated_question, dict)
                else reformulated_question
            )
    ...
    def log_session(
        prompt: str = Field(
            ...,
            description="A summary of the whole conversation: the user goal, steps taken, and final outcome.",
        ),
        output: str = Field(
            ..., description="The agent's response, generated code, or result summary."
        ),
        feedback: str = Field(
            "",
            description="Optional feedback or notes about the session outcome (e.g. 'worked', 'failed', 'needed adjustment').",
        ),
        category: str = Field(
            "",
            description="Optional category label for the session (e.g. 'acquisition', 'analysis', 'troubleshooting', 'question').",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
        ...
        try:
            if database_agent is None:
                result = {
                    "status": "skipped",
                    "reason": "Database agent not available (Elasticsearch not configured)",
                }
                return result
    ...
    def retrieve_session_logs(
        query: str = Field(
            ...,
            description="The current task or question — used to find semantically similar past sessions.",
        ),
    ...
    def answer_no_coding_query(user_query: str = Field(..., description="The user original query")):
        start_time = time.time()
        result = {"user_query": user_query, "no_coding_query": True}
        execution_time_ms = (time.time() - start_time) * 1000
        if benchmark_logger and user_query:
            benchmark_logger.set_query(user_query)
        if benchmark_logger:
    ...
    def get_microscope_events(
        limit: int = Field(
            100, description="Maximum number of recent events to retrieve (default 100)."
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
        ...
        try:
            ...
            result = {"status": "success", "event_count": len(events), "events": events}
            return result
        except Exception as e:
            result = {"status": "error", "message": str(e)}
            return result
    ...
    def get_last_microscope_event(
        event_type: str | None = Field(
            None, description="Filter by event type or None for any event."
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
        ...
        try:
            ...
            if event:
                result = {"status": "success", "event": event}
                return result
            else:
                result = {"status": "success", "event": None, "message": "No events found"}
                return result
        except Exception as e:
            result = {"status": "error", "message": str(e)}
            return result
    ...
    def viewer_screenshot(
        canvas_only: bool = Field(
            ...,
            description="If True, capture only the canvas (image data) without GUI elements. If False, include scale bars, labels, and other UI elements in the screenshot.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def viewer_layer_screenshot(
        layer_name: str = Field(
            ...,
            description="The exact name of the layer to capture. Use viewer_list_of_layers to see available layer names.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def get_layer_data(
        layer_name: str = Field(
            ...,
            description="Exact name of the layer to export. Use viewer_list_of_layers to see available names.",
        ),
        save_path: str | None = Field(
            None,
            description="Optional file path for the TIFF output. Defaults to /tmp/<sanitized_layer_name>.tif.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
        ...
        if viewer_proxy is not None:
            return viewer_proxy.call_on_main_thread(
                "get_layer_data", layer_name=layer_name, save_path=save_path
            )
        else:
    ...
    def view_image(
        ...
        overlay_path: str | None = Field(
            None,
            description="Optional path to a segmentation/label mask. Contours will be drawn as green outlines on the image.",
        ),
        frame_index: int = Field(
            0, description="For multi-frame TIFFs, which frame to display (0-indexed)."
        ),
        query: str = Field(
            "",
            description="Context about what to look for in the image. Returned alongside the image as text.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def viewer_add_image(
        data: NDArray | list[NDArray] | None = Field(
            None,
            description="Image data as a numpy array (2D, 3D, or 4D). For large data, prefer saving to TIFF and using 'path' instead.",
        ),
        path: str | None = Field(
            None,
            description="File path to load (TIFF, PNG, JPEG, etc.). Recommended for multi-dimensional data from execute_python_code workflows.",
        ),
        name: str | None = Field(
            None,
            description="Optional name for the image layer. If not provided, the filename will be used.",
        ),
        colormap: str | None = Field(
            None,
            description="Colormap to apply to the image (e.g., 'gray', 'viridis', 'magma', 'red', 'green', 'blue'). Default is 'gray' for grayscale images.",
        ),
        blending: str | None = Field(
            None,
            description="Blending mode for layer compositing: 'translucent' (default), 'additive', or 'opaque'.",
        ),
        channel_axis: int | str | None = Field(
            None,
            description="Axis index for multi-channel images. If provided, channels will be split into separate layers.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def viewer_add_labels(
        path: str | None = Field(
            None,
            description="File path to the labels image file (TIFF, PNG, etc.). Use this if loading from disk. Mutually exclusive with img_data.",
        ),
        img_data: NDArray | None = Field(
            None,
            description="Labeled mask as a 2D, 3D, or 4D integer array where each unique value represents a different region/object. Supports Z-stacks and time series. Mutually exclusive with path.",
        ),
        name: str | None = Field(
            None,
            description="Optional name for the labels layer in the viewer. If not provided, a default name will be generated.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
        ...
        if viewer_proxy is not None:
            return viewer_proxy.call_on_main_thread(
                "add_labels", path=path, img_data=img_data, name=name
            )
        else:
    ...
    def viewer_add_points(
        points: list[list[float]] = Field(
            ...,
            description="List of point coordinates. For 2D: [[y1, x1], [y2, x2], ...]. For 3D: [[z1, y1, x1], [z2, y2, x2], ...]. Coordinates are in pixel/voxel space.",
        ),
        ...
        if viewer_proxy is not None:
            return viewer_proxy.call_on_main_thread(
                "add_points", points=points, name=name, size=size
            )
        else:
    ...
    def viewer_remove_layer(
        name: str = Field(
            ...,
            description="The exact name of the layer to remove. Must match a layer name in the viewer.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def viewer_set_layer_properties(
        ...
        visible: bool | None = Field(
            None, description="Set layer visibility: True to show, False to hide."
        ),
        opacity: float | None = Field(
            None, description="Layer opacity from 0 (transparent) to 1 (opaque)."
        ),
        colormap: str | None = Field(
            None,
            description="Colormap name (e.g., 'gray', 'viridis', 'magma', 'red', 'green', 'blue').",
        ),
        blending: str | None = Field(
            None, description="Blending mode: 'translucent', 'additive', or 'opaque'."
        ),
        contrast_limits: list[float] | None = Field(
            None, description="Two-element list [min, max] for contrast/brightness adjustment."
        ),
        gamma: float | str | None = Field(
            None, description="Gamma correction value for exposure adjustment (typically 0.5-2.0)."
        ),
        ...
        if viewer_proxy is not None:
            ...
            return viewer.set_layer_properties(
                name=name,
                visible=visible,
                opacity=opacity,
                colormap=colormap,
    ...
    def viewer_reorder_layer(
        ...
        index: int | str | None = Field(
            None,
            description="Absolute position index (0 = bottom). Mutually exclusive with before/after.",
        ),
        before: str | None = Field(
            None,
            description="Name of layer to position this layer before. Mutually exclusive with index/after.",
        ),
        after: str | None = Field(
            None,
            description="Name of layer to position this layer after. Mutually exclusive with index/before.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
        ...
        if viewer_proxy is not None:
            return viewer_proxy.call_on_main_thread(
                "reorder_layer", name=name, index=index, before=before, after=after
            )
        else:
    ...
    def viewer_set_camera(
        center: list[float] | None = Field(
            None,
            description="Center position coordinates [y, x] for 2D or [z, y, x] for 3D to pan the camera to.",
        ),
        zoom: float | str | None = Field(
            None,
            description="Zoom level (larger values = more magnification). Typical range: 0.5 to 10+.",
        ),
        angle: float | str | None = Field(
            None, description="Rotation angle in degrees for 3D viewing mode."
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
        ...
        if viewer_proxy is not None:
            return viewer_proxy.call_on_main_thread(
                "set_camera", center=center, zoom=zoom, angle=angle
            )
        else:
    ...
    def viewer_set_ndisplay(
        ndisplay: int | str = Field(
            ...,
            description="Number of displayed dimensions: 2 for 2D view, 3 for 3D volumetric view.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def viewer_set_dims_current_step(
        axis: int | str = Field(
            ...,
            description="Axis identifier: integer index (0, 1, 2, ...) or axis name ('Z', 'T', 'C' for Z-stack, time, channel).",
        ),
        value: int | str = Field(
            ...,
            description="Step value (slice index) to navigate to along the specified axis. Must be within valid range for that dimension.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def viewer_set_grid(
        enabled: bool | str = Field(
            True, description="Enable (True) or disable (False) the pixel grid overlay."
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    def viewer_add_tracks(
        track_data: NDArray = Field(
            description="""NxD+1 NumPy Array or list containig the coordinates of N vertices with a
        ...
        features: dict[str, Any] | None = Field(
            None,
            description="Features table where each row corresponds to a point and each column is a feature.",
        ),
        tail_width: float | None = Field(
            None, description="Float value representing the width of the track tails in pixels."
        ),
        tail_length: float | None = Field(
            None,
            description="Float value representing the length of the positive (backward in time) tails in units of time.",
        ),
        user_query: str = Field(
            "", description="(Optional) The original user query, used for logging only."
        ),
    ...
    async def request_user_clarification(
        ...
        try:
            ...
            if result_elicit.action == "accept":
                ...
                result = {"agent_message": message, "user_answer": "Operation declined"}
            elif result_elicit.action == "cancel":
                result = {"agent_message": message, "user_answer": "Operation cancelled"}
