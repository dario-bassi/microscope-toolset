...
def compute_nc_ratio(image, nuclear_mask, cytoplasm_mask, background=None,
                     guard_band=0, pixel_size_um=None):
    ...
    Args:
        ...
        guard_band: int, pixels to erode from nuclear boundary and
            exclude from cytoplasm. Mitigates PSF blurring at the
            nuclear-cytoplasmic boundary. At 20x (PSF ~2px), use
            guard_band=2 for more accurate N:C ratios.
        pixel_size_um: float or None. When provided and guard_band is 0,
            automatically computes guard_band from PSF FWHM (~0.5µm):
            guard = max(0, round(0.5 / pixel_size_um)). This ensures
            PSF compression is mitigated at any magnification.
    ...
    # Auto guard_band from pixel size when not explicitly set
    if guard_band == 0 and pixel_size_um is not None:
        psf_fwhm_um = 0.5
        guard_band = max(0, int(round(psf_fwhm_um / pixel_size_um)))

    if background is not None:
        image = image - background

    # Apply guard band: erode nuclear mask and exclude boundary from cytoplasm
    if guard_band > 0:
        from scipy.ndimage import binary_erosion
        struct = np.ones((2 * guard_band + 1, 2 * guard_band + 1), dtype=bool)
        # Erode each nucleus independently to preserve labels
        eroded_nuc = np.zeros_like(nuclear_mask)
        for lbl in np.unique(nuclear_mask):
            if lbl == 0:
                continue
            region = nuclear_mask == lbl
            eroded = binary_erosion(region, structure=struct)
            if eroded.sum() > 0:
                eroded_nuc[eroded] = lbl
        nuclear_mask = eroded_nuc

        # Exclude boundary zone from cytoplasm mask
        boundary = ndimage.binary_dilation(nuclear_mask > 0, structure=struct)
        boundary = boundary & ~(nuclear_mask > 0)
        cytoplasm_mask = cytoplasm_mask.copy()
        cytoplasm_mask[boundary] = 0
