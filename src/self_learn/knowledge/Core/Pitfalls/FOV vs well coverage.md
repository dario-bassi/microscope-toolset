...
find cell counts well below the expected value. Hill fits flatten,
dose-response curves compress, EC50 estimates drift.

## The cause

The experiment seeds cells across a physical
...
```python
from self_learn.hardware.core import set_objective
from self_learn.hardware.config import refresh_config
...
- **Droplet detection at 20×**: LoG at 20x undercounts densely-packed sub-cellular features (lipid droplets, puncta) because individual features fuse into large blobs. Go to 40× to resolve individuals, or use an intensity-based proxy (mean fluorescence per cell; fraction of cell area above a threshold) that is fusion-robust.
- **Dose-response non-monotonicity**: a dip in the middle of a dose titration is almost always a segmentation artifact, not a biological effect — unless the expected response is genuinely biphasic. Check FOV coverage across wells first.

## See also

- [[Core/Approach/Acquisition strategy]] — magnification selection decision table.
- [[Core/Strategies/Multi-scale morphometry]] — two-pass approach for counting + morphology.
