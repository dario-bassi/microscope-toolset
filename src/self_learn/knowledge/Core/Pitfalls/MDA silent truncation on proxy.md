...
This pattern has been observed on FRAP recovery curves (missing late frames tightens the apparent half-life) and long timelapse acquisitions where frame loss occurs during a network hiccup.
...
Use `utils/mda_diagnostics.py:run_events_checked(core, events, expected_frames=N, on_frame=cb)` (sprint #15). It returns `(results, report)` where `report.is_truncated` is the cross-check the bare `run_events` is missing:
...
- `utils/mda_diagnostics.py` — the `run_events_checked` countermeasure and `MDAFrameTruncation` report dataclass.
...
- `utils/render_vs_submit.py` — agent-side numerical guard that catches the *downstream* effect (re-derived value disagrees with submitted value because of missing frames).
