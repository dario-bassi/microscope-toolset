...
Save annotated images to a temporary directory for the LLM to read.

> **Cross-platform:** Use Python's `tempfile` module instead of hardcoding `/tmp/` (which doesn't exist on Windows).
> ```python
> import tempfile
> from pathlib import Path
> tmp = Path(tempfile.gettempdir())  # works on Windows, macOS, Linux
> ```
...
# Then Read the saved PNG with Claude's vision
```
...
for i, obj in enumerate(ambiguous_objects):
    ...
    Image.fromarray(crop).save(tmp / f"object_{i}.png")
```
...
6. **Use descriptive filenames**: e.g., `tmp / "experiment_bf_overlay.png"`, not `tmp / "img.png"`.
