...
- [[Core/Approach/Backward design]] — the design discipline this applies.
- [[Core/Concepts/Exposure and photodamage]] — dose budget for long runs.
- [[Core/Strategies/Closed-loop autofocus]] — Z-drift correction during a timelapse.
- [[Core/Strategies/Gentle imaging]] — dose minimisation patterns.
- [[Core/Strategies/Wave propagation]] — timelapse design for wave-like dynamics.

## Literature

- [[Papers/Almada 2019]] —
  live-to-fixed protocols let a single timelapse span live dynamics and
  fixed-state super-resolution on the same cells by triggering fixation
  mid-experiment via programmable fluid exchange.
- [[Papers/McDole 2018]] — for long
  timelapses on motile / growing specimens (embryos, organoids, developing
  zebrafish), the imaging volume must not be fixed at t=0. A scout scan
  each time point re-measures the specimen's centroid and bounding box,
  and the tile grid + Z range are regenerated around the updated bbox
  with a fixed margin — otherwise the specimen clips out of the acquired
  volume as it develops.
