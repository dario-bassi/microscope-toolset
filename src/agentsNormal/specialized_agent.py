...
class DatabaseAgent(BaseAgent):
    ...
    def _retrieve_api_information(self, reformulated_query: str) -> list | str:
        try:
            ...
            api_docs_result = self.es_client.hybrid_search(
                index_name=self.api_collection,
            ...
            sorted_list_api_docs_result = sorted(
                list_api_docs_result, key=lambda x: x["score"], reverse=True
            )

            # merge all result into a single list
            merged_list = []
            # iterate through the first list
            merged_list = self._rerank_and_add_to_list(
                merged_list=merged_list,
                partial_result_list=sorted_list_api_docs_result,
                keyword_field="contextualize_text",
                query=reformulated_query,
            )
    ...
    def _retrieve_api_micromanager_information(self, reformulated_query: str) -> list | str:
        try:
            ...
            micromanager_docs_results = self.es_client.hybrid_search(
                index_name=self.micromanager_collection,
                query=reformulated_query,
                query_vector=query_embedded,
            ...
            # sort list of results
            sorted_list_micromanager_docs_results = sorted(
                list_micromanager_docs_results, key=lambda x: x["score"], reverse=True
            )
    ...
    def _retrieve_pdfs_information(self, reformulated_query: str) -> list | str:
        try:
            ...
            pdf_docs_results = self.es_client.hybrid_search(
                index_name=self.pdf_collection,
                query=reformulated_query,
                query_vector=query_embedded,
                keyword_to_search_for_bm25="content",
            )
            ...
            # sort list of results
            sorted_list_pdf_docs_results = sorted(
                list_pdf_docs_results, key=lambda x: x["score"], reverse=True
            )
    ...
    def devices_micromanager_context(self, query: str, reformulated_query: str) -> dict[str, ...]:
        ...
        list_devices_micromanager_result = self._retrieve_api_micromanager_information(
            reformulated_query
        )
    ...
    def retrieve_session_logs(self, query: str, k: int = 5) -> list:
        ...
        try:
            ...
            results = self.db_log.query_by_vector(
                collection_name=self.db_log_name, vector=query_embedded, k=k
            )
    ...
    def split_query_in_keyword(self, query: str):
        ...
        return self.call_agent(
            ...
            input_user=history,
            error_string=error_description,
            output_format=ExtractKeywordOutput,
        )
    ...
    def _rerank(self, query: str, chunk: str):
        ...
        try:
            ...
            inputs = self.tokenizer(
                query, chunk, return_tensors="pt", truncation=True, max_length=512
            )
            self.model.eval()
            with torch.no_grad():
    ...
    def _rerank_and_add_to_list(
        self, merged_list: list, partial_result_list: list, keyword_field: str, query: str
    ):
        """
